...
# ==========================================
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Categories", "Words", "User Permissions","Leaderboard Tools", "📜 Activity Log"])
    ...
    with tab1:
        ...
        if cats:
            for c in cats:
                ...
                col1.write(f"📁 **{c['name'].capitalize()}**")

                # Delete logic for Category
                if col2.button("🗑️ Delete", key=f"del_cat_{c['id']}"):
                    st.session_state.confirm_delete_cat = c['id']


            if 'confirm_delete_cat' in st.session_state:
                target_id = st.session_state.confirm_delete_cat
                st.warning(f"⚠️ Deleting this category will delete ALL words inside it. Are you sure?")
                if st.button("Yes, Delete Category and All Words"):
                    res = requests.delete(
                        f"{API_BASE_URL}/categories/{target_id}",
                        params={"admin_name": st.session_state.username}
                    )
                    if res.status_code == 200:
                        st.success("Category wiped out.")
    ...
    with tab2:
        ...
        try:
            ...
            word_data = res.json() if res.status_code == 200 else []
        except requests.exceptions.RequestException:
            st.error("Could not connect to the API.")
            word_data = []

        # 2. Display and Handle Actions outside the fetch try-block
        if word_data:
            for cat in word_data:
                st.markdown(f"#### {cat['category_name'].capitalize()}")
                for w in cat['words']:
                    col_w, col_btn = st.columns([4, 1])
                    col_w.write(f"• {w['word']} → 💡 {w['hint']}")

                    # Delete logic
                    if col_btn.button("❌", key=f"del_word_{w['id']}"):
                        del_res = requests.delete(
                            f"{API_BASE_URL}/words/{w['id']}",
                            params={"admin_name": st.session_state.username}
                        )
                    ...
                    # Show Promote button only if they are a player
                        if current_role == "player":
                            if st.button("🚀 Promote to Admin", use_container_width=True):
                                res = requests.put(
                                    f"{API_BASE_URL}/auth/promote/{selected_user}", 
                                    params={"admin_name": st.session_state.username}
                                )
                                if res.status_code == 200:
                        ...
                        # Show Demote button only if they are an admin
                            ...
                            if st.button("📉 Demote to Player", use_container_width=True):
                                res = requests.put(
                                    f"{API_BASE_URL}/auth/demote/{selected_user}", 
                                    params={"admin_name": st.session_state.username}
                                )
                                if res.status_code == 200:
    ...
    with tab4:
        ...
        if not st.session_state.confirm_reset:
            if st.button("🚨 Reset Entire Scoreboard", type="primary", use_container_width=True):
                res = requests.delete(
                    f"{API_BASE_URL}/game/reset-scores", 
                    params={"admin_name": st.session_state.username}
                )
                if res.status_code == 200:
    ...
    with tab5:
        ...
        st.info("This log is immutable and records all administrative changes.")
        
        res = requests.get(f"{API_BASE_URL}/auth/logs")
        if res.status_code == 200:
            logs = res.json()
            if logs:
                st.dataframe(logs, use_container_width=True, hide_index=True)
                
                # Export to CSV for "Sir Amir's" reports
                import pandas as pd
                ...
                st.download_button(
                    ...
                    data=csv,
                    file_name=f"hangman_audit_log_{time.strftime('%Y%m%d')}.csv",
                    mime='text/csv',
                )
            else:
